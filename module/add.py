...
def address_get():
    ...
    if p_date is not None:
        json_data = list(
            filter(lambda item: p_date.lower() in item["date"].lower(), json_data)
        )
    if p_schedule is not None:
        ...
        json_data = list(
            filter(
                lambda item: p_schedule.lower() in item["schedule"].lower(), json_data
            )
        )
    if p_time is not None:
        json_data = list(
            filter(lambda item: p_time.lower() in item["time"].lower(), json_data)
        )
